...
| deviceCode | a202006152b | String(20) | 否 | 门禁编码-28181 的 20 位编码要求，则第 11-13 位固定使用 195 由中心编码（行政区划）（8）、行业编 码（2）、设备类型（3）、网络标识（1）、 设备序号（6）构成 行业编码和网络标识见字典数据
...
| community | xxx村 | String(36) | 上传类型为省实有人口时非空 | 设备所在社区/村
| policeDistrict | xxxx派出所 | String(36) | 上传类型为省实有人口时非空 | 设备所在警务区
| unitCode | 123123 | String(12) | 是 | 传送单位代码
